...
def apply_in_kernel_rope(
  ...
  """Applies Rotary Position Embedding (RoPE) in VMEM with 128-lane fused FMA.

  Args:
    x: Activation tensor. Shape [B, H_kv, T, D] or [B, H_kv, T, G, D].
    positions: Token positions of shape [B, T].
    cfgs: RpaConfigs with model RoPE configurations.

  Returns:
    Rotated activation tensor with identical shape and dtype as x.
  """
  if not cfgs.model.apply_rope:
    return x

  rope_dim = cfgs.model.actual_rope_dim
  half_dim = rope_dim // 2
  theta = cfgs.model.rope_theta
  ordering = cfgs.model.rope_ordering

  # Frequency coefficients: shape [half_dim]
  dim_indices = lax.broadcasted_iota(jnp.int32, (half_dim,), 0) * 2
  inv_freq = 1.0 / (
      theta ** (dim_indices.astype(jnp.float32) / float(rope_dim))
  )

  # positions: [B, T] -> angles: [B, T, half_dim]
  ...
  if rope_dim == x.shape[-1]:
    ...
    x_rope = x_f32[..., :rope_dim]
    x_pass = x_f32[..., rope_dim:]
    has_pass = True

  if ordering == "split":
    # Fused 128-lane full-width vector rotation (avoids unaligned sub-lane slicing)
    ...
    x_swap = jnp.concatenate(
        [x_rope[..., half_dim:rope_dim], x_rope[..., :half_dim]], axis=-1
    )
  ...
  if has_pass:
    x_out = jnp.concatenate([x_rot, x_pass], axis=-1)
  else:
    x_out = x_rot
...
def rpa_body(
  ...
  if cfgs.model.apply_rope:
    ...
    q_5d_rot = apply_in_kernel_rope(q_5d, positions_q, cfgs=cfgs)
    q = q_5d_rot.reshape(
        cfgs.batch_size,
        cfgs.model.num_kv_heads,
        cfgs.bq_sz * cfgs.aligned_num_q_heads_per_kv_head,
  ...
  if cfgs.model.apply_rope:
    ...
    positions_k = jnp.stack(processed_kv_len, axis=0)[:, None] + tok_iota_k
    if cfgs.serve.kv_layout == configs.KVLayout.SEQ_ALONG_LANE:
      k_t = k.transpose(0, 1, 3, 2)
      k_rot_t = apply_in_kernel_rope(k_t, positions_k, cfgs=cfgs)
      k = k_rot_t.transpose(0, 1, 3, 2)
    else:
      k = apply_in_kernel_rope(k, positions_k, cfgs=cfgs)
    ...
    def _run(final_allocs, schedule_ref, dma_sem, scratches):
      ...
      def execute_schedule_chunk(start_step, num_steps):
        ...
        for h, s in zip(flat_hbm, flat_smem):
          mem_space = getattr(jax.typeof(h), "memory_space", None)
          if mem_space == pltpu.HBM:
            ...
            read_size = pl.multiple_of(read_size, 128)
