...
class SmemArrayOfStructs(SmemWrapper):
  ...
  @classmethod
  def create_shape_dtype(cls, shape, struct_cls, struct_size):  # type: ignore
    return cls(
...
class RpaSchedule:
  ...
  def mask_out_steps(self, step, _, *, b_idx):
    """Mask out unvisited metadata schedule entries at (step, b_idx)."""
    self.s_idx[step, b_idx] = -1
    self.q_idx[step, b_idx] = 0
    self.k_idx[step, b_idx] = 0
    self.is_last_k[step, b_idx] = 0
    self.do_writeback[step, b_idx] = 0

    self.dma_q[step, b_idx, 0] = 0
    self.dma_q[step, b_idx, 1] = 0

    for i in range(self.cfgs.bkv_p_cache):
      self.dma_kv_cache[step, b_idx, i, 0] = 0
      self.dma_kv_cache[step, b_idx, i, 1] = 0
      self.dma_kv_cache[step, b_idx, i, 2] = 0

    for i in range(self.cfgs.bkv_p_new):
      dma_entry = self.dma_kv_new[step, b_idx, i]
    ...
    self.total_wait_kv_in[step] = 0
    self.total_wait_kv_out[step] = 0
    self.total_wait_q_in[step] = 0
    self.total_wait_o_out[step] = 0
...
def compute_metadata(
  ...
  def q_loop(q_idx, hbm_offset, *, s_idx, q_start, q_end, k_len, q_len, num_k):
    ...
    def do_flush():
      ...
      for b_idx in range(cfgs.batch_size):
        ...
        mask_step_fn = functools.partial(schedule.mask_out_steps, b_idx=b_idx)
        jax.lax.fori_loop(start_step, cfgs.max_steps_ub, mask_step_fn, None)

      _compute_waits(schedule, 0, cfgs.max_steps_ub, cfgs=cfgs)
    ...
    new_hbm_offset, curr_ptr = jax.lax.cond(will_exceed, do_flush, no_flush)
...
def rpa_metadata_schedule_kernel(
  ...
  schedule_ref.actual_steps[0] = jnp.where(
      max_steps > 1, hbm_offset + max_steps, max_steps
  )

  # Step 3: Mask out unvisited steps.
  for b_idx in range(cfgs.batch_size):
    start_step = lane_lengths_ref[b_idx]
    mask_step_fn = functools.partial(schedule_ref.mask_out_steps, b_idx=b_idx)
    jax.lax.fori_loop(start_step, max_steps, mask_step_fn, None)

  # Step 4: Write back results to HBM.
  _compute_waits(schedule_ref, 0, max_steps, cfgs=cfgs)
  _write_schedule_to_hbm(
      schedule_ref,
      schedule_hbm_ref,
      hbm_offset,
