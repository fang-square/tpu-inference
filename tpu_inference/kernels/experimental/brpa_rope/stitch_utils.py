...
def _stitch_decode_lane(
  ...
  merged_dst_vreg = jax.lax.select(
      lane_idx >= dst_rel, rolled_src_vreg, dst_vreg
  )
...
def store_new_kv_lane(
  ...
  if cfgs.block.bq_sz == 1:
    ...
    strided_vmem_ref[pl.ds(dst_chunk_idx, outer_dim, lanes_per_col)] = (
        merged_dst_vreg
    )
  ...
  * page_size]
  ...
  if cfgs.block.bq_sz == 1:
    return _stitch_decode_lane(
        vmem_u32_ref, bkv_sz_cache, cache_pages, new_tok_offset, v_len
    )
  else:
