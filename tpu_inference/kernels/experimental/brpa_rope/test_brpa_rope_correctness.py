...
def test_in_kernel_rope_correctness():
  ...
  print(f"  Token-Major vs Ref: Max diff: {max_diff:.6e}, Mean diff: {mean_diff:.6e}")
...
def test_kv_group_major_correctness():
  ...
  q_kv_major = (
      q_raw_token_major.reshape(T, H_kv, G, D).transpose(1, 0, 2, 3)
  )

  # Reference attention output
  q_rot_ref = apply_reference_rope(q_raw_token_major, theta=theta, ordering="split")
  k_rot_ref = apply_reference_rope(k_raw, theta=theta, ordering="split")
  attn_ref = reference_attention(q_rot_ref, k_rot_ref, v_raw, sm_scale=sm_scale)
  ...
  attn_brpa_reconstructed = (
      attn_brpa_kv.transpose(1, 0, 2, 3).reshape(T, H_q, D)
  )

  diff = jnp.abs(attn_brpa_reconstructed - attn_ref)
  max_diff = float(jnp.max(diff))
  mean_diff = float(jnp.mean(diff))
  print(f"  KV-Group Major vs Ref: Max diff: {max_diff:.6e}, Mean diff: {mean_diff:.6e}")
  assert max_diff < 1e-4, f"Max diff {max_diff} exceeded tolerance"
  print(">>> TEST 2 PASSED <<<")
...
def test_qkv_pipeline_equivalence():
  ...
  w_qkv_fp8, scale_w_qkv = qkv_pipeline.quantize_q_kv_major_weight_to_fp8(w_qkv_merged)
...
def test_strided_dma_correctness():
  ...
  k_rot_ref = apply_reference_rope(k_raw, theta=theta, ordering="split")
  attn_ref = reference_attention(
      q_rot_ref, k_rot_ref, v_raw, sm_scale=sm_scale
  )
...
def main(_):
  ...
  print("\n==================================================")
  print("ALL CORRECTNESS AND NUMERICAL TESTS PASSED (4/4)")
  print("==================================================")
